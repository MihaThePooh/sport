...
def add(home, move, sets, when, kind=NORMAL, note="", append=False):
    ...
    # Комментарий переживает перезапись подходов. Правка присылает только
    # числа, и без этого исправленная опечатка стирала бы текст, который
    # человек писал отдельной кнопкой.
    if было and not note:
        note = note_of(было)
    if append and было:
...
def note_set(home, move, when, note):
    ...
    if цель is None:
        # Занятия ещё нет — заводим его ради комментария. Дату
        # подтвердили, значит человек сегодня тренируется и пишет
        # заметку по ходу дела: «турник мокрый», «болит плечо». Ждать
        # первой записанной серии, чтобы разрешить ему это, нет причин.
        #
        # Пустой комментарий записи не создаёт: открыл поле, передумал,
        # закрыл — в журнале не должно остаться занятия-призрака.
        if not текст:
            return
        rows.append({"дата": when, "упражнение": move, "подходы": "",
                     "всего": 0, "формат": NORMAL, "комментарий": текст})
        write(home, rows)
        return

    цель["комментарий"] = текст
    write(home, rows)
