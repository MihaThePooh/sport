...
HEAD = ["дата", "упражнение", "подходы", "всего", "формат"]
    ...
    for m in MOVES:
        ...
        out["движения"][m] = {
            ...
            "последнее": ({"дата": last["дата"], "подходы": sets_of(last),
                           "сумма": total_of(last),
                           "формат": kind_of(last)} if last else None),
            "занятий": len(mine),
        }
    return out


def add(home, move, sets, when, kind=NORMAL):
    ...
    if kind == MYO and len(sets) < MIN_MYO_SETS:
