...
def full(x, y):
    """То же, но без прозрачных углов — заливаем их цветом плитки.

    Это и есть маскируемая иконка. Система накладывает СВОЮ форму:
    круг, квадрат со скруглением — свою на каждой прошивке. Если под
    её вырезом окажется прозрачность, лаунчер подложит туда белое, и
    вокруг значка появится белая рамка — ровно то, что и вышло.

    Поэтому цвет обязан доходить до самого края квадрата, а рисунок —
    держаться в середине: срезать могут до 20% с каждой стороны."""
    return v1(x, y) or GREEN
